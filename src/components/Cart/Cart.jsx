...
const Cart = (props) => {
  ...
  const submitOrderHandler = async (userData) => {
    ...
    const response = await fetch('https://food-order-react-app-ab677-default-rtdb.europe-west1.firebasedatabase.app/orders.json', {
      method: 'POST',
      body: JSON.stringify({
        user: userData,
        order: cartCtx.items
  ...
  const modalContent = <>
    {cartItems}   
      ...
      {isCheckingOut && <Checkout onCancel={closeCheckout}
  ...
  const didSubmitModalContent = <p>Order submitted, thank you!</p>
